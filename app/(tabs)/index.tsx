        ...
        <View style={styles.section}>
            ...
            <View style={styles.assetList}>
                {(() => {
                  ...
                  return (
                    <>
                      <AssetItem img="https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/solana/info/logo.png" name="Solana" sym="SOL" bal={Number(solWallet.balance > 0 ? solWallet.balance : (isOnChainReady && 'SOL' in onChainBalances ? onChainBalances['SOL'] : (userProfile?.saldo_sol || 0))).toFixed(4)} price={prices.SOL} change={priceChanges.SOL} onPress={() => router.push({ pathname: '/wallet', params: { coin: 'SOL' } } as any)} />
                      <AssetItem img="https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/assets/0xdAC17F958D2ee523a2206206994597C13D831ec7/logo.png" name="Tether" sym="USDT" bal={Number(isOnChainReady && 'USDT' in onChainBalances ? onChainBalances['USDT'] : (userProfile?.saldo_usdt || 0)).toFixed(2)} price={prices.USDT} change={priceChanges.USDT} onPress={() => router.push({ pathname: '/wallet', params: { coin: 'USDT' } } as any)} />
                      <AssetItem img="https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/assets/0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48/logo.png" name="USDC" sym="USDC" bal={Number(isOnChainReady && 'USDC' in onChainBalances ? onChainBalances['USDC'] : (userProfile?.saldo_usdc || 0)).toFixed(2)} price={prices.USDC} change={priceChanges.USDC} onPress={() => router.push({ pathname: '/wallet', params: { coin: 'USDC' } } as any)} />
                      <AssetItem img={require('../../public/BDC.png')} name="BodeCoin" sym="BDC" bal={Number(isOnChainReady && 'BDC' in onChainBalances ? onChainBalances['BDC'] : (userProfile?.saldo_bdc || 0)).toFixed(2)} price={prices.BDC} change={priceChanges.BDC} onPress={() => router.push({ pathname: '/wallet', params: { coin: 'BDC' } } as any)} />
                      <AssetItem img="https://gateway.lighthouse.storage/ipfs/bafkreig4gwqmpwrvai3boloziuzwxhr4yhadkyxrbofxw4wzmccxtkrw3q" name="Escoteiros" sym="ESCT" bal={Number(isOnChainReady && 'ESCT' in onChainBalances ? onChainBalances['ESCT'] : (userProfile?.saldo_esct || 0)).toFixed(2)} price={prices.ESCT} change={priceChanges.ESCT} onPress={() => router.push({ pathname: '/wallet', params: { coin: 'ESCT' } } as any)} />
                      <AssetItem img="https://gateway.lighthouse.storage/ipfs/bafybeihjtb3bae57rzlh4hblksaswxwfgjs4jxwsbeoj6yh5sfl7qso65q" name="Brutos" sym="BRT" bal={Number(isOnChainReady && 'BRT' in onChainBalances ? onChainBalances['BRT'] : (userProfile?.saldo_brt || 0)).toFixed(2)} price={prices.BRT} change={priceChanges.BRT} onPress={() => router.push({ pathname: '/wallet', params: { coin: 'BRT' } } as any)} />
                    </>
                  );
                })()}
                ...
                {dynamicTokens.map((tk) => (
                  <AssetItem 
                    ...
                    onPress={() => router.push({ pathname: '/wallet', params: { coin: tk.symbol } } as any)}
                  />
                ))}
            </View>
        </View>
