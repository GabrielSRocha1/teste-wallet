...
export const translateError = (error: any, t: TFn): string => {
  ...
  if (
    ...
    return t('A cotação expirou enquanto a transação era enviada. Tente novamente — vamos buscar uma nova cotação.');
  }
  ...
  if (
    ...
    return t('O preço mudou mais do que a tolerância (0,5%). Tente novamente; se persistir, reduza o valor.');
  }
  ...
  if (
    ...
    return t('Esta é a primeira vez que você recebe este token. Será criada uma conta SPL (~0,002 SOL). Confirme novamente.');
  }
  ...
  if (
    ...
    return t('Saldo insuficiente. Confirme que você tem SOL para a taxa de rede (~0,005) além do valor do swap.');
  }

  if (lower.includes('transaction simulation failed') || lower.includes('simula')) {
    return t('A rede recusou a transação na pré-validação. Atualize a cotação e tente novamente.');
  }

  if (lower.startsWith('timeout') || lower.includes('timeout:') || lower.includes('não confirmada após')) {
    return t('A rede está congestionada. Verifique seu histórico antes de tentar de novo — a transação pode ainda confirmar.');
  }

  // Dicionário genérico — chaves em inglês (vindas do erro original em lower-case),
  // valores são chaves de tradução (em pt-base).
  ...
  return message || t('Ocorreu um erro inesperado.');
};
